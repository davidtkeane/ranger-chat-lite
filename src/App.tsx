...
function App() {
    ...
    const SLASH_COMMANDS = [
        ...
        {
            cmd: '/ai status',
            desc: 'Check local AI servers',
            icon: '🤖',
            submenu: ['status', 'check']
        },
        {
            cmd: '/model',
            ...
            icon: '🧠',
            args: '<name>',
            submenu: ['llama3', 'mistral', 'gemma', 'phi3'] // Could be dynamic later
        },
        {
            cmd: '/ask',
    ...
    // ============================================
    // AI INTELLIGENCE STATE (v2.2.0)
    // ============================================
    const [ollamaUrl, setOllamaUrl] = useState(() => localStorage.getItem('rangerChatOllamaUrl') || 'http://localhost:11434')
    const [lmStudioUrl, setLmStudioUrl] = useState(() => localStorage.getItem('rangerChatLmStudioUrl') || 'http://localhost:1234')
    const [currentAiModel, setCurrentAiModel] = useState(() => localStorage.getItem('rangerChatAiModel') || '')
    const [isThinking, setIsThinking] = useState(false)
    const [aiStatus, setAiStatus] = useState<{ ollama: boolean; lmstudio: boolean }>({ ollama: false, lmstudio: false })

    // Persist AI settings
    useEffect(() => {
        localStorage.setItem('rangerChatOllamaUrl', ollamaUrl)
        localStorage.setItem('rangerChatLmStudioUrl', lmStudioUrl)
        localStorage.setItem('rangerChatAiModel', currentAiModel)
    }, [ollamaUrl, lmStudioUrl, currentAiModel])
...
💡 What you can do:
   ...
   • Or configure a custom server in Settings`,
    ...
    // Handle AI commands (v2.2.0)
    const handleAiCommand = async (fullCommand: string) => {
        const cmd = fullCommand.trim()

        // /ai status
        if (cmd === '/ai status' || cmd === '/ai check') {
            setMessages(prev => [...prev, {
                type: 'system',
                sender: 'System',
                content: '🔍 Checking AI server status...',
                timestamp: new Date().toLocaleTimeString()
            }])

            const ollamaRes = await apiTestingService.testLocalLLM('ollama', ollamaUrl)
            const lmStudioRes = await apiTestingService.testLocalLLM('lmstudio', lmStudioUrl)

            setAiStatus({
                ollama: ollamaRes.success,
                lmstudio: lmStudioRes.success
            })

            let statusMsg = "🤖 **AI Server Status:**\n"
            statusMsg += `• **Ollama:** ${ollamaRes.success ? '✅ ONLINE' : '❌ OFFLINE'} (${ollamaUrl})\n`

            let foundModel = '';
            if (ollamaRes.success) {
                const models = (ollamaRes.data?.models || []).map((m: any) => m.name)
                statusMsg += `  *Models: ${models.join(', ') || 'None'}*\n`
                if (models.length > 0) foundModel = models[0]
            }

            statusMsg += `• **LM Studio:** ${lmStudioRes.success ? '✅ ONLINE' : '❌ OFFLINE'} (${lmStudioUrl})`
            if (lmStudioRes.success) {
                const models = (lmStudioRes.data?.data || []).map((m: any) => m.id)
        ...
        // /model <name>
        const modelMatch = cmd.match(/^\/model\s+(.+)$/i)
        if (modelMatch) {
            ...
            setMessages(prev => [...prev, {
                type: 'system',
                sender: 'System',
                content: `🧠 AI Model set to: **${newModel}**`,
                timestamp: new Date().toLocaleTimeString()
            }])
            return
        }

        // /ask <query> or /ai <query>
        const askMatch = cmd.match(/^(\/ask|\/ai)\s+(.+)$/i)
        if (askMatch) {
            ...
            if (!currentAiModel) {
                setMessages(prev => [...prev, {
                    type: 'system',
                    sender: 'System',
                    content: '⚠️ No AI model selected. Use `/model <name>` or check `/ai status` to see available models.',
                    timestamp: new Date().toLocaleTimeString()
                }])
                return
            }

            // Show user message first (if desired, or just system response)
            // Ideally we want to show it as a User message, but sendMessage usually clears input.
            // Let's just trigger the AI response.
            ...
            // Determine provider based on URL or just try both?
            // For now specific to the current plan, let's assume if model name contains 'llama' or 'mistral' it might catch either.
            // But usually we need to know which provider the model belongs to. 
            // Simplified: Try Ollama first if online, then LM Studio? 
            // Or just try the one that has the model? 
            // Let's try Ollama then LM Studio.

            try {
                // Try Ollama
                let response = await apiTestingService.chatLocalLLM('ollama', ollamaUrl, currentAiModel, query)

                // If error or empty, try LM Studio
                if (response.startsWith('Error:') || response === 'No response') {
                    const lmResponse = await apiTestingService.chatLocalLLM('lmstudio', lmStudioUrl, currentAiModel, query)
                    if (!lmResponse.startsWith('Error:') && lmResponse !== 'No response') {
                        response = lmResponse
                    }
                }
                ...
                setMessages(prev => [...prev, {
                    type: 'system',
                    sender: 'AI', // Or 'Ollama'/'Bot'
                    content: `🤖 **${currentAiModel}:**\n${response}`,
                    timestamp: new Date().toLocaleTimeString()
                }])
            } catch (e) {
                setMessages(prev => [...prev, {
                    type: 'system',
                    sender: 'System',
                    content: `❌ AI Error: ${(e as Error).message}`,
                    timestamp: new Date().toLocaleTimeString()
                }])
            } finally {
                setIsThinking(false)
    ...
    const sendMessage = async () => {
        ...
        if (trimmedInput.startsWith('/ai') || trimmedInput.startsWith('/model') || trimmedInput.startsWith('/ask')) {
            // Close slash menu immediately
            ...
            {view === 'chat' && (
                <div className={`chat-interface ${radioSettings.chatScreensaverEnabled ? 'screensaver-active' : ''}`}>
                    ...
                    <div className="chat-input-area">
                        {/* AI Status Indicator (small dot) */}
                        {(aiStatus.ollama || aiStatus.lmstudio) && (
                            <div
                                className="ai-status-dot"
                                title={`AI Active: ${aiStatus.ollama ? 'Ollama' : ''} ${aiStatus.lmstudio ? 'LM Studio' : ''}`}
                            ></div>
                        )}
                        <button
                ...
                view === 'settings' && (
                    <div className="settings-screen">
                        ...
                        <div className="settings-content">
                            ...
                            {/* AI Intelligence Settings Section (v2.2.0) */}
                            <div className="settings-section ai-settings-section">
                                <h3>🤖 AI Intelligence</h3>
                                <p className="settings-note">Configure local AI providers. RangerChat Lite connects directly to your local instances.</p>
                                ...
                                <div className="setting-item">
                                    ...
                                    <div className="server-input-row">
                                        ...
                                        />
                                        <div className={`status-dot ${aiStatus.ollama ? 'online' : 'offline'}`} title={aiStatus.ollama ? 'Online' : 'Offline'}></div>
                                    </div>
                                </div>
                                ...
                                <div className="setting-item">
                                    ...
                                    <div className="server-input-row">
                                        ...
                                        />
                                        <div className={`status-dot ${aiStatus.lmstudio ? 'online' : 'offline'}`} title={aiStatus.lmstudio ? 'Online' : 'Offline'}></div>
                                    </div>
                                </div>
                                ...
                                <div className="setting-actions">
                                    <button
                                        ...
                                        onClick={() => handleAiCommand('/ai status')}
                                    >
                                        🔄 Check Connections
                                    </button>
                                </div>
