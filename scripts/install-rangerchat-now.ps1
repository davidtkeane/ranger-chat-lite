...
function Write-Banner {
 ...
 ======================================================================
            ...
            Version 2.0.0 - One-Command Installer (Windows)
...
# Check Node.js version (warn if v23+, recommend v22 LTS)
try {
    $nodeVer = node -v 2>$null
    if ($nodeVer) {
        $majorVer = [int]($nodeVer -replace '^v','').Split('.')[0]
        if ($majorVer -ge 23) {
            Write-Color "WARNING: Node.js $nodeVer detected. Some native modules may not work with v23+." -Color Yellow
            Write-Color "  Recommended: Node.js v22 LTS (https://nodejs.org)" -Color Yellow
        }
    }
} catch {}

# Check Python (required for node-gyp / native modules on Windows)
$pythonInstalled = $false
try {
    $pythonVersion = python --version 2>$null
    if ($pythonVersion) {
        Write-Color "[OK] $pythonVersion (needed for native modules)" -Color Green
...
if (-not $pythonInstalled) {
    Write-Color "Python not found (optional - needed for some native modules)" -Color Yellow
    if ($wingetAvailable) {
        Write-Color "  Install with: winget install Python.Python.3.12" -Color Cyan
    } else {
        Write-Color "  Download from: https://www.python.org/downloads/" -Color Cyan
    }
}
...
try {
    ...
    Write-Color "Could not update npm (v$currentNpm). Continuing..." -Color Yellow
    Write-Color "  Error: $_" -Color DarkGray
}
...
if (Test-Path $InstallDir) {
    ...
    git clone https://github.com/davidtkeane/ranger-chat-lite.git $InstallDir
...
try {
    npm install 2>&1 | Tee-Object -Variable npmInstallResult
    if ($LASTEXITCODE -ne 0) {
        throw "npm install exited with code $LASTEXITCODE"
    }
    Write-Color "[OK] Dependencies installed" -Color Green
} catch {
    Write-Color "npm install failed!" -Color Red
    Write-Color "Common Windows fixes:" -Color Yellow
    Write-Color "  1. Install Python: winget install Python.Python.3.12" -Color Cyan
    Write-Color "  2. Install Build Tools: npm install -g windows-build-tools" -Color Cyan
    Write-Color "  3. Use Node 22 LTS: winget install OpenJS.NodeJS.LTS" -Color Cyan
    Write-Color "  4. Clear npm cache: npm cache clean --force" -Color Cyan
    Write-Color "  5. Delete node_modules and retry: Remove-Item node_modules -Recurse -Force; npm install" -Color Cyan
    Write-Color "`nError details:" -Color DarkGray
    Write-Color "  $_" -Color DarkGray
    Write-Color "`nTry fixing the issue above and run: cd $InstallDir; npm install" -Color Yellow
}
