...
const getRelayPath = (): string => {
    ...
    // Development: relay in lib/rangerblock (standalone repo)
    return path.join(__dirname, '..', 'lib', 'rangerblock', 'relay-server-bridge.cjs')
}
...
const cryptoUtilsPath = app.isPackaged
    ...
    : path.join(__dirname, '..', 'lib', 'rangerblock', 'crypto-utils.cjs')
...
const ledgerPath = app.isPackaged
    ...
    : path.join(__dirname, '..', 'lib', 'rangerblock', 'ledger-service.cjs')
...
const fileTransferPath = app.isPackaged
    ...
    : path.join(__dirname, '..', 'lib', 'rangerblock', 'file-transfer-service.cjs')
...
const walletPath = app.isPackaged
    ...
    : path.join(__dirname, '..', 'lib', 'rangerblock', 'wallet-ledger-integration.cjs')
...
const VERSIONS_URL = 'https://raw.githubusercontent.com/davidtkeane/ranger-chat-lite/main/versions.json'
