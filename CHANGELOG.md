...
## [2.0.0] - 2026-02-01 - "Standalone Release"

### Breaking Changes
- **Standalone Repository**: RangerChat Lite is now its own GitHub repo at `davidtkeane/ranger-chat-lite`
- **Bundled Libraries**: All 15 rangerblock `.cjs` modules now live in `lib/rangerblock/` (no parent repo dependency)
- **Simplified Install**: Install scripts now `git clone` the standalone repo directly (no sparse checkout needed)

### Changed
- `electron/main.ts`: All `../../..` parent repo paths replaced with `../lib/rangerblock/` local paths
- `package.json`: Version 2.0.0, repository URL points to `ranger-chat-lite`, publish repo updated
- `README.md`: All URLs updated to new repo, added Troubleshooting section, added Migration note
- Install scripts (`.sh` and `.ps1`): Simplified to direct `git clone`, version bumped to 2.0.0

### Added (Windows Installer Fixes)
- **Python check**: Detects Python and suggests `winget install Python.Python.3.12` if missing
- **Node.js version warning**: Warns if Node v23+ detected, recommends v22 LTS
- **npm install error handling**: Shows common Windows fixes (Python, Build Tools, Node 22, npm cache)
- **npm update error visibility**: No longer silently hides npm update failures

### Migration Guide
If you previously installed from `rangerplex-ai`:
1. Delete old installation: `rm -rf ~/RangerChat-Lite`
2. Install fresh: `git clone https://github.com/davidtkeane/ranger-chat-lite.git ~/RangerChat-Lite`
3. Run: `cd ~/RangerChat-Lite && npm install && npm run dev`
4. Your identity and wallet data in `~/.rangerblock/` is preserved automatically
